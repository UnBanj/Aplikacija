...
export class OrderMailer {
    ...
    private makeOrderHtml(order: Order): string {
        let suma = order.cart.cartArticles.reduce((sum, curentCartArticle: CartArticle)=>{
              return sum + curentCartArticle.quantity * 
              curentCartArticle.article.articlePrices[curentCartArticle.article.articlePrices.length-1].price
        }, 0);
